...
async function route(request: Request) {
  ...
  return mcpHandler({ jwksUrl: `${origin}/api/auth/jwks`, verifyOptions: { issuer: `${origin}/api/auth`, audience: resource } }, async (req, jwt) => {
    ...
    return createMcpHandler((server) => {
      server.registerTool("list_skills", { title: "List team skills", description: "List every skill saved by the authenticated user's organizations", inputSchema: {} }, async () => ({ content: [{ type: "text", text: JSON.stringify(await listUserSkills(jwt.sub!), null, 2) }] }))
