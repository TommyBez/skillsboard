            ...
            <div className="mt-9 grid gap-4 lg:grid-cols-2">
              {section.entries.map((entry) => (
                <Link
                  key={entry.path}
                  href={entry.path}
                  // `/guides/[slug]` has no route-level shell to share — the page
                  // renders straight from `params`, so the reusable shell Partial
                  // Prefetching would hand out is empty. Per-link prefetching is
                  // what makes a guide click instant: it pulls the slug's
                  // prerendered payload from the static cache, no server
                  // invocation. Measured at 150ms latency: 75ms to the guide h1
                  // with this, 418ms without. It costs this page's prefetch
                  // budget, and the guides index is where that earns out.
                  prefetch
