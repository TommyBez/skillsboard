...
import type {
  VercelSkillsDefinition,
  VercelSkillsInlineLink,
} from "@/lib/seo/vercel-skills"
import { buildResourceArticleSchema } from "@/lib/seo/resource-article-schema"
import { resourcePaths } from "@/lib/seo/resources"
import { siteConfig } from "@/lib/site"

function InlineLink({ link }: { link: VercelSkillsInlineLink }) {
  return (
...
export function VercelSkillsPage({ entry }: { entry: VercelSkillsDefinition }) {
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-[1000px] px-5 py-12 md:px-10 md:py-16">
        ...
        <section aria-labelledby="meanings-heading" className="pt-16">
          <SectionHeading
            eyebrow="01 / Disambiguation"
            id="meanings"
            title={entry.meanings.title}
            intro={entry.meanings.intro}
          />
          <SectionTable
            caption="The separate things published under the same two words, and which one you can install."
            columns={entry.meanings.columns}
            rows={entry.meanings.rows}
            labelWidth="w-[22%]"
          />
          <NoteList notes={entry.meanings.notes} />
          <InlineLink link={entry.meanings.link} />
          <SectionSources
            sourceIds={entry.meanings.sourceIds}
            sources={entry.sources}
          />
        </section>
        ...
        <section aria-labelledby="catalog-heading" className="pt-16">
          ...
          />
          <SectionTable
            caption="Every skill in the collection, what it does, and what its own frontmatter says about licensing."
        ...
        <section aria-labelledby="names-heading" className="pt-16">
          <SectionHeading
            eyebrow="03 / Naming"
            id="names"
            title={entry.names.title}
            intro={entry.names.intro}
          />
          <SectionTable
            caption="The folder each skill lives in against the name declared inside it."
            columns={entry.names.columns}
            rows={entry.names.rows}
            labelWidth="w-[34%]"
          />
          <NoteList notes={entry.names.notes} />
          <InlineLink link={entry.names.link} />
          <SectionSources
            sourceIds={entry.names.sourceIds}
            sources={entry.sources}
          />
        </section>

        <section aria-labelledby="weight-heading" className="pt-16">
          <SectionHeading
            eyebrow="04 / Cost"
            id="weight"
            title={entry.weight.title}
            intro={entry.weight.intro}
          />
          <SectionTable
            caption="What landed on disk, and which part of it every session pays for."
            columns={entry.weight.columns}
            rows={entry.weight.rows}
            labelWidth="w-[24%]"
          />
          <NoteList notes={entry.weight.notes} />
          <InlineLink link={entry.weight.link} />
          <SectionSources
            sourceIds={entry.weight.sourceIds}
            sources={entry.sources}
          />
        </section>

        <section aria-labelledby="caution-heading" className="pt-16">
          <SectionHeading
            eyebrow="05 / Read first"
            id="caution"
            title={entry.caution.title}
            intro={entry.caution.intro}
          />
          <SectionTable
            caption="The skills that reach outside their own folder, and the decision each one asks of you."
            columns={entry.caution.columns}
            rows={entry.caution.rows}
            labelWidth="w-[22%]"
          />
          <NoteList notes={entry.caution.notes} />
          <InlineLink link={entry.caution.link} />
          <SectionSources
            sourceIds={entry.caution.sourceIds}
            sources={entry.sources}
          />
        </section>

        <section aria-labelledby="drift-heading" className="pt-16">
          <SectionHeading
            eyebrow="06 / Drift"
            id="drift"
            title={entry.drift.title}
            intro={entry.drift.intro}
          />
          <SectionTable
            caption="Claims made by the packaging, against what the files in the repository contain."
            columns={entry.drift.columns}
            rows={entry.drift.rows}
            labelWidth="w-[20%]"
          />
          <NoteList notes={entry.drift.notes} />
          <InlineLink link={entry.drift.link} />
          <SectionSources
            sourceIds={entry.drift.sourceIds}
            sources={entry.sources}
          />
        </section>
        ...
        <section aria-labelledby="install-heading" className="pt-16">
          ...
          <CodeBlock
            ...
            copy={{
              buttonLabel: "Copy commands",
              ariaLabel: "Copy the install commands",
              copiedAriaLabel: "Install commands copied",
            }}
          />
          <SectionSources
        ...
        <section aria-labelledby="sources-heading" className="pt-16">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-relaxed text-muted-foreground">
            ...
            GitHub API, the files themselves, the directory, Vercel's own
            documentation, and one run of the installer against an empty
            project. Where nothing is published, this page says so instead of
            filling the gap. Collections change, so check the linked sources
            before you rely on a count.
          </p>
