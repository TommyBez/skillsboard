...
export function AddSkillDialog({
  ...
  /* Set only when the save just took this team from an empty library to a
     stocked one and the server judged the invite ask eligible. */
  const [inviteStepTeamId, setInviteStepTeamId] = useState<string | null>(null)
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      ...
      if (result.inviteTeammateStep) setInviteStepTeamId(result.inviteTeammateStep.teamId)
    ...
    {inviteStepTeamId ? (
      <FirstSkillInviteStep
        ...
        onOpenChange={(nextOpen) => {
          if (!nextOpen) setInviteStepTeamId(null)
        }}
        teamId={inviteStepTeamId}
      />
    ) : null}
    </>
  )
