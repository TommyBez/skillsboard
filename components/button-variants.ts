...
/**
 * The button class recipe, mirrored from the vendored `components/ui/button.tsx`
 * (shadcn) on purpose: that file imports `@base-ui/react` at module scope, so
 * importing `buttonVariants` from it pulls the Base UI runtime into the bundle.
 * Marketing surfaces (the landing page) apply these same classes to plain
 * anchors and native buttons instead, keeping ~18 kB (gzipped) of Base UI out
 * of the landing's first-load JS. If the vendored recipe changes, change it
 * here too — same classes, same paint.
 */
