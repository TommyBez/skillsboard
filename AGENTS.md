...
- TypeScript 7 is a native binary and no longer exports `transpileModule`/`ModuleKind` from the `typescript` package entry. The unit tests in `tests/` therefore type-strip with `stripTypeScriptTypes` from `node:module` — reach for that instead of importing `typescript` in a test. It is why `engines.node` is `>=22.13.0`. The editor-only `{ "name": "next" }` tsconfig plugin is also a TypeScript 5 feature; `tsc --noEmit` ignores plugins, so type checking is unaffected.
