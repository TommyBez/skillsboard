...
- `next.config.ts` turns on `partialPrefetching`, so `next dev` reports any navigation that cannot show instant UI ("Next.js encountered runtime data during prerendering"). The fix is to wrap the data access in `<Suspense>` or mark it `'use cache'` — `params` needs a boundary even when `generateStaticParams` covers every value. `export const instant = false` is not a general escape hatch; per the `instant` reference it exists to exempt navigations passing through a blocking *ancestor* of a segment you have marked instant.
- A `notFound()` raised inside a `<Suspense>` boundary lands after the response has committed to 200, so it reads as a soft 404. Under Cache Components every dynamic route streams a shell first, which is why the guide-slug existence check lives in `proxy.ts` — that is where a real 404 status can still be set. Keep such checks to cheap in-memory lookups.
