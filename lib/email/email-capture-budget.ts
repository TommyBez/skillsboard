 ...
 * wiring, down to which environment variables the client is built from.
 *
...
/**
 * What the wiring logs, once per process, when this environment has no Redis
 * credentials to build a limiter from.
 *
 * It names both pairs because which one a reader should set depends on where
 * they are. `Redis.fromEnv()` reads the canonical Upstash names first, which
 * is what a database created in the Upstash console is reached with, and falls
 * back to the pair the Vercel Marketplace integration writes into the project
 * by itself, under the names the older Vercel KV product used.
 *
 * The read-only token that integration also writes is named nowhere, here or
 * in the wiring: a claim increments a counter, and that is a write.
 */
export const MISSING_CAPTURE_CREDENTIALS_WARNING =
  "Email capture is not rate limited: set UPSTASH_REDIS_REST_URL and " +
  "UPSTASH_REDIS_REST_TOKEN for an Upstash database configured by hand, or " +
  "KV_REST_API_URL and KV_REST_API_TOKEN, which the Vercel Upstash " +
  "integration writes by itself"
