...
export const anthropicSkills: AnthropicSkillsDefinition = {
  ...
  answerNotes: [
    "The three sets do not sync with each other, and Anthropic says so directly. The overview page states that custom skills do not sync across surfaces, that a skill uploaded to claude.ai must be uploaded separately to the API, and that Claude Code skills are filesystem-based and separate from both. The same separation applies to the first-party skills: the pre-built document skills are documented as not available in Claude Code, and the thirteen Claude Code bundled skills are not in the repository.",
  ...
  sets: {
    ...
    notes: [
      ...
      "Read the marketplace manifest rather than counting folders. The five plugins declared in .claude-plugin/marketplace.json do not cover every folder in the skills directory: canvas-design and frontend-design and the rest of example-skills are listed, but the manifest is edited when a skill ships, so a folder can exist before a plugin references it. The manifest is the file that decides what /plugin install actually gives you.",
    ],
    link: {
  ...
  team: {
    ...
    body: [
      "Every one of these skills is installed per person, per surface. Someone enables theme-factory on claude.ai, someone else installs example-skills in Claude Code, and a third person is on the API where neither is reachable. Anthropic's own documentation says custom skills do not sync across surfaces and that claude.ai has no org-wide distribution for them, so the drift is designed in rather than accidental.",
      ...
      "Skills Board is a shared library for that smaller layer: the set your team actually recommends, in one searchable place, with the source repository and path visible on every entry, so a teammate can open the original SKILL.md before installing anything anywhere.",
    ],
    paths: [
      {
    ...
    limits: [
      ...
      "It follows the latest version available from the saved source, and does not pin or preserve historical versions. The repository above changed twice in the two days before we wrote this page.",
