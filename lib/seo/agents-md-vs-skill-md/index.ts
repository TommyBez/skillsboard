...
export const agentsMdVsSkillMd: AgentsMdVsSkillMdDefinition = {
  ...
  intro: [
    "Both are Markdown files, both sit in your repository, and both tell a coding agent what to do. That is where the similarity ends. AGENTS.md describes one project. A SKILL.md folder packages one capability. Treating them as interchangeable produces the two failure modes teams actually hit: a repository file so long that half of it is truncated or ignored, and a skill nobody can reuse because it assumes your directory layout.",
  ...
  answer:
    "AGENTS.md is project context: one Markdown file with no required fields, read before the agent starts work and applied to everything it does in that part of the tree. SKILL.md is a packaged capability: a folder with YAML frontmatter and instructions that loads only when the agent judges it relevant or you invoke it. Most repositories want both, and they do not overlap.",
  answerNotes: [
