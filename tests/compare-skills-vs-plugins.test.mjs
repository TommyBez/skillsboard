...
test("the plugins comparison mounts its own four CTA locations", async () => {
  assert.equal(entry.ctaLocation, "compare_skills_plugins")
  ...
  assert.ok(
    layout.includes("compare_skills_plugins_header"),
  ...
  for (const suffix of ["header", "hero", "inline", "closing"]) {
    assert.ok(
      events.includes(`"compare_skills_plugins_${suffix}"`),
      `landing_cta_clicked cannot report compare_skills_plugins_${suffix}`,
    )
  }
})
