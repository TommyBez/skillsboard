...
const comparePath = "/compare/claude-skills-vs-slash-commands"
...
test("the slash commands comparison is registered everywhere a comparison is addressed", () => {
  assert.equal(entry.path, comparePath)
  assert.equal(comparePaths.skillsVsSlashCommands, comparePath)
...
test("the slash commands comparison is listed in the static llms.txt", async () => {
...
test("the comparison answers the query with the documented details", () => {
