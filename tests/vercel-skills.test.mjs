...
test("the page mounts its own four CTA locations", async () => {
  ...
  assert.ok(
    layout.includes("vercel_skills_header"),
  ...
  for (const suffix of ["hero", "inline", "closing"]) {
    assert.ok(
      page.includes(`vercel_skills_${suffix}`),
      `the page never renders the vercel_skills_${suffix} CTA`,
    )
  }
  ...
  for (const suffix of ["header", "hero", "inline", "closing"]) {
    assert.ok(
      events.includes(`"vercel_skills_${suffix}"`),
      `landing_cta_clicked cannot report vercel_skills_${suffix}`,
    )
  }
})
