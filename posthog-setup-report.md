...
| `$pageview` | Canonical route view. Use `$pathname=/connect` for the MCP guide and `$pathname=/start` for first-run steps; authenticated views carry the active `team_id`. | `components/posthog-analytics.tsx` |
