...
11. If `pseo_learning` is absent or `pseo_learning.next_slot_at <= now`, no new repository PR was created in this run, no overlapping pSEO cluster PR exists, `current_uncheckpointed + candidate_count <= 4`, and one or two candidates from one cluster pass the qualitative publication gate, create one focused experimental-page PR. The first run after this contract initializes an absent slot as due immediately. Missing, zero, or `null` quantitative demand is prioritization evidence, not a veto. A transient PR/resource conflict leaves the slot overdue for the next heartbeat; an opened PR or no quality-qualified candidate records the outcome and advances `next_slot_at` by seven days.
...
- `pseo_learning` with `last_slot_at`, `next_slot_at`, outcome, definition hash, current problem-cluster lock, and a registry of page experiments containing `problem_cluster_id`, `canonical_intent_id`, URL, hypothesis, PR/deploy references, publication time, T+3/T+7/T+14/T+28 due and completed timestamps, measurement status, iterations, lifecycle state, and rollback reason. Creation persists both IDs before publication; recovery, deduplication, and sibling expansion read the per-experiment IDs rather than relying on the current lock;
