...
Every individual send freezes immutable intent, deterministic logical key, payload hash, opaque recipient reference, dry-run result, and provider-enforced idempotency key. The executor may hold the exact recipient transiently only for the selected send; the parent and durable state retain only its opaque reference or keyed hash. Within Resend's 24-hour window, allow at most three total attempts for the identical payload: one initial send plus at most two retries, and only for network failure, 429, 500, or concurrent-idempotent-request. Before either retry, exact idempotency-key or exact-resource status readback must prove the prior request had no effect; otherwise do not retry. Do not retry other 4xx responses or payload conflicts. After 24 hours without a provider ID, mark `delivery_ambiguous`, consume cap/cooldown, and never auto-resend. Missing the required idempotency-key input or exact-resource status readback keeps individual Pulse sends unavailable.
