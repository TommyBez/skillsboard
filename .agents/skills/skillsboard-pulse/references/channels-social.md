...
Owned social is a near-zero-cost candidate family. Synthesize every distinct truthful useful unit with an explicit plausibly positive contribution path through acquisition, activation, retention, or reactivation to the governing `AAT-28` objective. Emit its finite list, provider effect keys, schedule/resource interference, and explicit content or timing conflicts into the shared `pulse.scheduler` model; exact provider deduplication is not the only possible collision. Execute the scheduler's deterministic compatible selection through the official capability. There is no per-run publication quota, and completing one unit does not make another positive compatible unit ineligible. Missing analytics, team activation maturity, prior queue content, or quantitative evidence changes confidence but never blocks synthesis or publication.
