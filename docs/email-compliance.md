...
- Delivery is at most once, and deliberately so. `emailAutomationSend` is claimed before the provider call, keyed by person and message, so an overlapping or repeated run writes nothing and sends nothing. A refusal that the provider answered releases the claim for a later run. A request that throws without an answer keeps it, because the message may already have been accepted and a second copy of an email is a worse failure than one that never arrives.
