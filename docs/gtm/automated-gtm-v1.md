...
One weekly router reads every stage, validates the data, selects at most one primary diagnostic, and executes the next bounded action. Problem-led pSEO research is a protected sensing lane: it completes at least once every seven days even when another stage is routed, refreshes only the evidence backlog, and cannot create a second strategic constraint or implementation action. Active experiments and rollouts receive a separate daily monitor.
...
The module has two modes. The protected research pass completes at least once every seven days, normally inside the Monday strategic pulse; if it is overdue, the first subsequent heartbeat catches it up. It uses `programmatic-seo` and `content-strategy` to refresh the deduplicated evidence backlog from the product-marketing contract, current routes and canonical intents, public SERPs, official vendor documentation, primary research, attributable public problem signals, Search Console when connected, downstream PostHog outcomes, and optionally DataForSEO. The research pass is sensing, not a second router: it may update evidence, source availability, demand status, and candidate decisions, but it does not open a PR, publish a page, or displace the one primary routed action. An unrelated open PR does not block this research.

The operational diagnostic may turn a researched candidate into the run's single primary action only when the router selects Acquisition or that exact pSEO candidate as its one primary constraint. It uses `seo-audit` before any PR and inherits every repository, quality, measurement, and rollback gate below. A pSEO implementation never runs as an additive second action.
...
- **Body:** discover official PostHog plugin capabilities; verify project identity; reconcile canonical PostHog assets; calculate all five scorecard rows and `ΔAAT` from valid values; run the protected pSEO research pass when due and refresh only its evidence backlog; exclude unavailable, broken, or immature stages; route a verified defect first; route Revenue/sustainability next only for a proven cap breach or due review; otherwise apply downstream health gates before Acquisition scaling and select among eligible growth stages by absolute teams affected and strength of evidence. Execute one bounded action and monitor active experiments daily.
...
The pulse executes only one primary routed action per strategic run, while existing experiments receive independent monitoring and due pSEO research remains independent sensing. Eligible work includes a PostHog reconciliation or experiment, a connected-channel action inside configured caps, a repository-fixable product or Tracking QA defect, and a focused pSEO PR that passes its evidence and quality checks. The protected research pass does not authorize a PR or publication by itself. Broken Tracking QA blocks only dependent growth actions and permits direct repair of Pulse-owned analytics resources or an evidenced tracking PR.
...
|---|---|---|---|
| Acquisition | Weekly scorecard plus a protected pSEO evidence refresh at least every seven days, independent of the routed stage and unrelated locks. The branch maintains at most 30 deduplicated seeds and five shortlisted opportunities. | Research-only backlog update; when routed as the single primary action, one bounded channel/content action, one experimental-page PR from strong qualitative evidence, or at most three pages in one intent cluster when quantitative demand passes. | Missing research sources mark only those inputs unavailable. Qualification or attribution gaps block source-scale conclusions; weak product fit, canonical overlap, thin content, an overlapping resource lock, or failed SEO/verification blocks implementation. |
