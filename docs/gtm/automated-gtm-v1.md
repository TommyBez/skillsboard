...
**Date:** 2026-07-22
**Status:** Official PostHog plugin-first autonomous control loop with a protected weekly pSEO publish-to-learn lane; production Tracking QA remains incomplete
...
One weekly router reads every stage, validates the data, selects at most one primary diagnostic, and executes the next bounded action. Problem-led pSEO is a protected learning lane: research completes at least once every seven days, and one focused experimental-page PR containing at most two pages from the same problem cluster may start in each seven-day learning slot without waiting for Acquisition to become the routed constraint or for quantitative demand to exist. This lane does not create a second strategic constraint, may not create a second repository PR in the same run, and remains subject to independent PR approval, quality, measurement, resource-lock, and rollback checks. Active experiments and rollouts receive a separate daily monitor.
...
The module has three parts. The protected research pass completes at least once every seven days, normally inside the Monday strategic pulse; if it is overdue, the first subsequent heartbeat catches it up. It uses `programmatic-seo` and `content-strategy` to refresh the deduplicated evidence backlog from the product-marketing contract, current routes and canonical intents, public SERPs, official vendor documentation, primary research, attributable public problem signals, Search Console when connected, downstream PostHog outcomes, and optionally DataForSEO. The research pass is sensing: it may update evidence, source availability, demand status, and candidate decisions, but it does not itself open a PR or publish a page. An unrelated open PR does not block this research.

The protected learning slot is also due at least once every seven days. When one or two candidates from the same problem cluster pass the qualitative publication gate, the pulse may create one focused experimental-page PR without waiting for the full-funnel router to select Acquisition and without treating missing, zero, or `null` keyword-volume data as a veto. The slot uses `seo-audit` before the PR and requires distinct intent, product fit, current attributable sources, differentiated utility, truthful product-contract-safe claims, canonical safety, useful page-specific content, metadata, internal links, sitemap and indexation handling, measurement, and a reversible removal or `noindex` path. A run may create only one new repository PR across the primary router and this learning slot. A transient repository conflict retains the overdue slot for the next heartbeat; a completed `no_action` because no candidate passes quality advances it by seven days.

The routed pSEO diagnostic remains the scaling lane. It may expand a winning intent family or make a larger template change as the run's single primary action after early page evidence exists. No intent cluster may have overlapping implementation PRs, and no more than four indexable experimental pages may remain live without at least one completed 14-day checkpoint.
...
Field availability is not proof of demand. Store `demand_gate=pass|fail|unavailable|broken` separately: a valid zero is available but cannot pass, and CPC, keyword difficulty, Google Ads competition, or search-intent classification never satisfy the gate. The pulse owns and versions the rule over demand measures such as search volume, its monthly source series, or target-query Search Console impressions. Until market/language, formula, comparison window, minimum completeness, and threshold are recorded, the gate is `unavailable`; credentials alone never constitute demand. The gate prioritizes candidates and controls multi-page scaling, but it is not a prerequisite for the bounded qualitative learning slot.

Each research run keeps at most 30 deduplicated seed queries and shortlists at most five opportunities. A `canonical_intent_id` combines normalized locale, audience/problem, and intent independently of format; candidates, existing canonical URLs, and open PRs are deduplicated on that ID before shortlisting. A separate `problem_cluster_id` combines normalized locale and the shared audience/problem family, so a learning PR may contain only candidates with the same deterministic cluster ID and its repository resource lock uses that ID. Every candidate must have a distinct intent, a natural `create a team library` conversion path, current attributable sources, differentiated utility such as an original comparison, workflow, checklist, template, or decision framework, product-contract-safe claims, no canonical overlap, and complete metadata, internal-link, sitemap, indexation, and supported-schema handling.

Qualitative evidence may justify one focused PR containing at most two experimental pages from one problem cluster when the publication gate passes. Quantitative demand or positive deployed-page evidence may justify at most three additional indexable pages in that cluster through the routed scaling lane. The PR is the sole human checkpoint: no separate pilot approval or approval key exists.

After independent approval, merge, and deployment, evaluate without blocking the next non-overlapping weekly learning slot:

- **T+3 days:** verify deployment, canonical URL, sitemap discovery, crawlability, indexation state, and runtime health; repair a technical blocker immediately.
- **T+7 days:** read Search Console query/page discovery and impressions plus pageviews and page-attributable intent when trustworthy. Correct-intent impressions keep the page active; wrong-intent impressions trigger one bounded title, introduction, or targeting revision; zero impressions on an indexed healthy page is inconclusive and does not block the next experiment.
- **T+14 days:** complete the first learning decision. Growing or correct-intent impressions may authorize a routed sibling expansion; mismatched queries trigger one repositioning iteration; a healthy zero-impression page remains observable through T+28 while another cluster may enter the weekly slot.
- **T+28 days:** retain or expand a page with useful query discovery; consolidate, `noindex`, or retire a page that remained indexed with zero useful impressions after one material iteration, unless it has documented non-SEO product utility. Pause an intent pattern after two comparable 28-day misses and diagnose it before producing more.

Immediate rollback remains mandatory for factual or product-contract errors, privacy risk, broken routes, accidental indexation, or confirmed canonical conflict. Use page-specific CTA/signup intent only after a stable landing identifier such as `landing_path` and its query are implemented and validated; the current `landing_cta_clicked.location` describes the CTA surface, not the landing page. Use activated teams only after attribution is operational, and apply checks only to available page-attributable measures.

**Measurement target:** normalized UTMs, anonymous landing CTA intent, signup context, and eventual activated teams by source. Raw intent is available now; qualified visitors and source-to-activation attribution remain unavailable until §11 is resolved. Refresh pSEO demand evidence and the learning-slot state at least every seven days regardless of the routed stage. Experimental publishing is bounded by the one-PR-per-run rule and exact resource locks rather than by the full-funnel stage router; larger expansion remains a primary routed action.
...
- **Body:** discover official PostHog plugin capabilities; verify project identity; reconcile canonical PostHog assets; calculate all five scorecard rows and `ΔAAT` from valid values; run the protected pSEO research pass when due, monitor deployed pSEO checkpoints, and evaluate the weekly learning slot; exclude unavailable, broken, or immature stages; route a verified defect first; route Revenue/sustainability next only for a proven cap breach or due review; otherwise apply downstream health gates before Acquisition scaling and select among eligible growth stages by absolute teams affected and strength of evidence. Execute one bounded primary action, then create at most one protected pSEO page PR only when no repository PR was created in that run, and monitor active experiments daily.
...
- **Output:** five AARRR rows, `ΔAAT`, cohort `n` and maturity, PostHog plugin and data quality, protected pSEO research and learning-slot status, source availability, checked and shortlisted counts, active page checkpoints and backlog decisions, one routed constraint, the executed primary action or exact `no_action`, any protected pSEO action, active rollouts, and any PR state.

### Autonomous execution and PR gate

The pulse executes only one primary routed action per strategic run, while existing experiments and deployed pSEO pages receive independent monitoring. The protected pSEO learning lane may additionally create one focused page PR per seven-day slot without becoming a second strategic constraint, but the run may create only one new repository PR in total. Eligible primary work includes a PostHog reconciliation or experiment, a connected-channel action inside configured caps, a repository-fixable product or Tracking QA defect, and a routed pSEO scale action. The research pass alone does not authorize publication; the separate qualitative publication gate does. Broken Tracking QA blocks only dependent growth actions and permits direct repair of Pulse-owned analytics resources or an evidenced tracking PR.
...
### Diagnostic modules and protected Acquisition lane

These are branches of Loop 0, not independent strategic routers. The protected pSEO research and learning lane stays active on its cadence independently of the routed stage; every other diagnostic remains dormant until routed. All branches inherit the same QA, resource locks, automatic guardrails, state, and stop conditions. The detailed procedures live in `.agents/loops/skillsboard-gtm-pulse.md`.

| Module | Check and trigger | Output | Key stop condition |
|---|---|---|---|
| Acquisition | Weekly scorecard plus protected pSEO research and one learning slot at least every seven days, independent of the routed stage and unrelated locks. The branch maintains at most 30 deduplicated seeds, five shortlisted opportunities, and a maximum projected post-PR total of four live pages without a completed 14-day checkpoint. | Research backlog update; one focused PR with at most two qualitative experimental pages from one problem cluster; when routed as the primary action, a bounded channel action or up to three sibling pages after quantitative or deployed-page evidence. | Missing demand data does not block the qualitative slot. Weak product fit, canonical overlap, thin content, inaccurate claims, an overlapping resource lock, projected post-PR uncheckpointed pages above four, another repository PR created in the same run, or failed SEO/verification blocks implementation. |
...
Client analytics sanitizes only automatic URL properties: hashes and non-UTM query parameters are removed, and invitation capability paths are canonicalized without dropping the pageview. SDK-owned properties and explicit custom-event properties remain intact. Autocapture and exception capture are restored to the original PostHog integration behavior, Session Replay remains controlled by the PostHog project, and Do Not Track is honored. Replay page and network URLs receive the same sanitizer; network bodies, headers, and the rendered invitation-link result are excluded because they can contain live credentials. Vercel Analytics applies the same canonical URL rule. Browser PostHog initialization depends only on the Production-scoped project token; the server SDK uses its native `disabled` option outside Vercel production, and Vercel Analytics renders only there. Server-side analytics is fail-open so analytics failure cannot convert a successful product mutation into a user-visible error. Consent, retention, and internal-user policy remain launch blockers rather than assumptions.
